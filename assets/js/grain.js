...
function init() {
				...
				sphere.scale.multiplyScalar( .64 );
				scene.add( sphere );
				...
				var material = new THREE.MeshStandardMaterial( { color: 0x3126f5, wireframe: true } );
			...
			function render() {
				...
				camera.position.x = Math.sin( timer ) * 2+1;
				//camera.position.y = Math.cos( timer ) * 2+2;
				camera.position.z = Math.cos( timer ) * 2+2;

				camera.lookAt( cameraTarget );

				square.rotation.y = timer * 4;
