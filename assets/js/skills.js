...
function init() {
				...
				tri.position.y = -0.50;
				tri.position.z = -0.4;
				tri.scale.multiplyScalar( 2.4 );
				scene.add( tri );

				// Sphere
				var geometry = new THREE.SphereGeometry( 0.5, 10, 10 );
				var material = new THREE.MeshBasicMaterial( {color: 0x3126f5,wireframe:true} );
			...
			function render() {
				...
				camera.position.z = Math.cos( timer ) * 4+2;

				camera.lookAt( cameraTarget );

				square.rotation.y = timer * 2;
				ring.rotation.x = timer * 2;
				tri.rotation.x = timer * 2;
